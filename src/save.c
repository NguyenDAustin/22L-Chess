...
static Rank parseRank(const char *s){
	if(strcmp(s, "KING") == 0) return KING;
	if(strcmp(s, "QUEEN") == 0) return QUEEN;
	if(strcmp(s, "ANTEATER") == 0) return ANTEATER;
	if(strcmp(s, "BISHOP") == 0) return BISHOP;
	if(strcmp(s, "KNIGHT") == 0) return KNIGHT;
	if(strcmp(s, "ROOK") == 0) return ROOK;
	if(strcmp(s, "PAWN") == 0) return PAWN;
	return EMPTY;
}
